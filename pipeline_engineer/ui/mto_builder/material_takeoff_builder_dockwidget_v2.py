...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'material_takeoff_builder_dockwidget_base_v2.ui'))
...
class MaterialTakeOffBuilderDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        self.mlcbFeatureLayer.layerChanged.connect(self.populate_feature_fields_tree)
        ...
        self.populate_feature_fields_tree()
        self.populate_layers_to_add_attributes_tree()
        ...
        self.pbFeatureSummary.clicked.connect(self.on_feature_summary_clicked)
    ...
    def populate_feature_fields_tree(self):
        self.twFeatureFields.clear()
        layer = self.mlcbFeatureLayer.currentLayer()
        
        for field in layer.fields():
            item = QTreeWidgetItem([field.name()])
            item.setCheckState(0, Qt.CheckState.Unchecked)
            self.twFeatureFields.addTopLevelItem(item)
    ...
    def populate_layers_to_add_attributes_tree(self):
        self.twPipelineFeatureLayers.clear()
        for layer in QgsProject.instance().mapLayers().values():
            if layer.type() == QgsMapLayer.VectorLayer and layer.geometryType() == QgsWkbTypes.PointGeometry:
                item = QTreeWidgetItem([layer.name()])
                item.setCheckState(0, Qt.CheckState.Unchecked)
                self.twPipelineFeatureLayers.addTopLevelItem(item)
    ...
    def on_feature_summary_clicked(self):
        
        feature_layer = self.mlcbFeatureLayer.currentLayer()
        pipe_layer = self.mlcbCorridorSummaryLayer.currentLayer()
        
        pipe_id_field = self.fcbLineIDField.currentField()
        
        fields_to_keep = [pipe_id_field]
        
        for i in range(self.twFeatureFields.topLevelItemCount()):
            item = self.twFeatureFields.topLevelItem(i)
            if item.checkState(0) == Qt.Checked:
                field = item.text(0)  # Assuming the field name is in column 0
                fields_to_keep.append(field)
        
        try:
            fields_to_keep.remove('corridor')
        except:
            pass
        ...
        corridor_named = processing.run("native:renametablefield", 
            {'INPUT':joined,
            'FIELD':f'{pipe_id_field}',
            'NEW_NAME':'corridor',
            'OUTPUT':'memory:'})['OUTPUT']
        ...
        retained_again.setName(f'{feature_layer.name()} Feature Summary')
        QgsProject.instance().addMapLayer(retained_again)
    ...
    def on_add_line_attributes_clicked(self):
        ...
        for i in range(self.twPipelineFeatureLayers.topLevelItemCount()):
            item = self.twPipelineFeatureLayers.topLevelItem(i)
            if item.checkState(0) == Qt.Checked:
                component = item.text(0)  # Assuming the field name is in column 0
                layers_to_merge.append(component)
        
        point_layer = processing.run("native:mergevectorlayers", 
                                                {'LAYERS':layers_to_merge,
                                                 'CRS':None,
                                                 'OUTPUT':'memory:'})['OUTPUT']
        
        point_layer = processing.run("native:deletecolumn", 
                                                {'INPUT':point_layer,
                                                 'COLUMN':['layer','path']
                                                 ,'OUTPUT':'memory:'})['OUTPUT']
        
        feature_exp = self.fewPointIDExpression.expression()
        assembly_exp = self.fewPointAssemblyExpression.expression()
        ...
        feature_id_added = processing.run("native:fieldcalculator", 
                                                {'INPUT':point_layer,
                                                 'FIELD_NAME':'feat_id',
                                                 'FIELD_TYPE':2,
                                                 'FIELD_LENGTH':0,
        ...
        assembly_added = processing.run("native:fieldcalculator", 
                                                {'INPUT':feature_id_added,
                                                 'FIELD_NAME':'assembly',
                                                 'FIELD_TYPE':2,
                                                 'FIELD_LENGTH':0,
                                                 'FIELD_PRECISION':0,
                                                 'FORMULA':assembly_exp,
                                                 'OUTPUT':'memory:'})['OUTPUT']
        
        service_added = processing.run("native:fieldcalculator", 
                                                {'INPUT':assembly_added,
                                                 'FIELD_NAME':'service',
                                                 'FIELD_TYPE':2,
                                                 'FIELD_LENGTH':0,
                                                 'FIELD_PRECISION':0,
        ...
        for field in service_added.fields():
        ...
        line_attributes_added = attach_pipe_attributes(point_layer=service_added,point_fields_containing_lines=corridor_attributes,
                            service=service_description, remove_line_service=True,
                           line_layer=line_layer,line_id_field=line_id_field,line_attributes_to_copy=[f'{service_description_lower}_material',f'{service_description_lower}_size',f'{service_description_lower}_class'])

        line_attributes_added.setName(f'{service_description} Line Attributes Added')
