...
def pressures_through_a_network(line_data_df, line_xyz_df,grids_df,
    ...
    while i < len(known_pressures_df):
        ...
        for _, line_row in lines_connected_df.iterrows():
            ...
            known_pressures_df = pd.concat(
                [known_pressures_df, pd.DataFrame({"junction": [from_junction], "p_bar": [line_from_junction_pres]})],
                ignore_index=True
            )
    ...
    master_results_df.to_clipboard()

    return known_pressures_df, updated_lines_df
